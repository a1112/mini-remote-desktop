...
$macosLocalDualScript = Get-Content -Path (Join-Path $scriptDir "run_local_dual_process_lan_canary_macos.sh") -Raw
...
Assert-True ($macosLocalDualScript -match 'h264\|hevc\|av1') "macOS local dual canary documents AV1 codec selection"
Assert-True ($macosLocalDualScript -match '--synthetic-cv-capture') "macOS local dual canary documents deterministic synthetic CV capture"
Assert-True ($macosLocalDualScript -match '--source-fit-profile') "macOS local dual canary documents source-fitted native profile selection"
Assert-True ($macosLocalDualScript -match 'native\|native120\|source-native') "macOS local dual canary exposes native source-fitted profile aliases"
Assert-True ($macosLocalDualScript -match 'MRD_LAN_E2E_SOURCE_FIT_PROFILE') "macOS local dual canary forwards source-fitted profile mode to Tauri autorun"
Assert-True ($macosLocalDualScript -match 'MRD_LAN_TEST_SYNTHETIC_CAPTURE') "macOS local dual canary enables synthetic CV capture via environment"
Assert-True ($macosLocalDualScript -match 'videotoolbox_av1') "macOS local dual canary uses VideoToolbox AV1 for AV1 codec selection"
Assert-True ($macosLocalDualScript -match 'software_av1') "macOS local dual canary uses software AV1 receiver decode"
Assert-True ($macosLocalDualScript -match 'isUnsupportedAv1CapabilityError|is_unsupported_av1_capability_error') "macOS local dual canary treats unavailable AV1 hardware as unsupported"
Assert-True ($macosLocalDualScript -match 'display_refresh_limited') "macOS local dual canary classifies local refresh caps explicitly"
Assert-True ($macosLocalDualScript -match 'display_refresh_limit_reason') "macOS local dual canary reports the local refresh cap reason"
Assert-True ($macosLocalDualScript -match 'DEFAULT_MACOS_RENDER_TARGET_FPS') "macOS local dual canary can infer the local render cap before pipeline metrics exist"
Assert-True ($macosLocalDualScript -match 'no_remote_frames') "macOS local dual canary treats early sample-threshold failures as display-limited when the render cap is below the requested FPS"
Assert-True ($macosLocalDualScript -match 'sender_capture_p95_ms') "macOS local dual canary summary reports sender capture latency"
Assert-True ($macosLocalDualScript -match 'sender_send_datagram_p95_ms') "macOS local dual canary summary reports sender datagram latency"
Assert-True ($macosLocalDualScript -match 'fresh_sender_frame_ratio') "macOS local dual canary summary reports fresh sender frame ratio"
Assert-True ($macosLocalDualScript -match 'sender_last_frame_error') "macOS local dual canary summary reports sender frame errors"
Assert-True ($macosLocalDualScript -match 'capture_permission_required') "macOS local dual canary classifies screen recording permission failures"
Assert-True ($macosLocalDualScript -match 'and not capture_permission_required') "macOS local dual canary keeps screen recording permission failures above refresh-limit classification"
Assert-True ($macosLocalDualScript -match 'capture_display_unavailable') "macOS local dual canary classifies display capture availability failures"
Assert-True ($macosLocalDualScript -match 'and not capture_display_unavailable') "macOS local dual canary keeps display availability failures above refresh-limit classification"
Assert-True ($macosLocalDualScript -match 'capture_source_width') "macOS local dual canary summary reports capture source width"
Assert-True ($macosLocalDualScript -match 'capture_source_height') "macOS local dual canary summary reports capture source height"
Assert-True ($macosLocalDualScript -match 'swap_chain_max_frame_latency') "macOS local dual canary summary reports render proxy drawable count"
Assert-True ($macosLocalDualScript -match 'queue_depth') "macOS local dual canary summary reports receiver queue depth"
Assert-True ($macosLocalDualScript -match 'localDualProcess') "macOS local dual canary records two-process run metadata"
Assert-True ($macosLocalDualScript -match 'distinct_service_processes') "macOS local dual canary proves distinct service process ids"
Assert-True ($macosLocalDualScript -match 'distinct_ipc_endpoints') "macOS local dual canary proves distinct IPC endpoints"
Assert-True ($macosLocalDualScript -match 'processCleanup') "macOS local dual canary records cleanup status"
Assert-True ($macosLocalDualScript -match 'ipc_endpoints_removed') "macOS local dual canary proves IPC sockets are removed"
Assert-True ($macosLocalDualScript -match 'media_verified') "macOS local dual canary summary reports media verification"
Assert-True ($macosLocalDualScript -match 'data_plane_verified') "macOS local dual canary summary reports data-plane verification"
Assert-True ($macosLocalDualScript -match 'display_native_surface_attached') "macOS local dual canary summary reports native display attachment"
