...
def _make_capture_patch(codec: str, roi_enable: bool, roi_rect: str, roi_qoffset: float) -> Dict[str, Any]:
...
def _mk_case(base_cfg: Dict[str, Any], *, transport: str, codec: str, roi_enable: bool, native_roi_probe: bool, roi_rect: str, roi_qoffset: float) -> Dict[str, Any]:
    ...
    analysis["capture_patch"] = _make_capture_patch(codec, roi_enable, roi_rect, roi_qoffset)
    env = dict(analysis.get("agent_env") or {})
    env["AGENT_NVENC_NATIVE_ROI_ENABLE"] = "1" if native_roi_probe else "0"
    analysis["agent_env"] = env
    return {
        "name": f"{transport}-{codec}-roi{'on' if roi_enable else 'off'}-native{'on' if native_roi_probe else 'off'}",
        "transport": transport,
        "codec": codec,
...
async def run_matrix(root: Path, base_cfg: Dict[str, Any], roi_rect: str, roi_qoffset: float) -> Dict[str, Any]:
    cases = [
        ...
        _mk_case(base_cfg, transport="webtransport", codec="hevc", roi_enable=True, native_roi_probe=False, roi_rect=roi_rect, roi_qoffset=roi_qoffset),
        _mk_case(base_cfg, transport="webtransport", codec="av1", roi_enable=True, native_roi_probe=False, roi_rect=roi_rect, roi_qoffset=roi_qoffset),
        _mk_case(base_cfg, transport="webtransport", codec="av1", roi_enable=True, native_roi_probe=True, roi_rect=roi_rect, roi_qoffset=roi_qoffset),
    ]
...
def write_report(root: Path, payload: Dict[str, Any]) -> Path:
    ...
    for r in payload.get("cases", []):
        key = f"{r.get('case_transport')}|{r.get('case_codec')}|{1 if r.get('case_roi_enable') else 0}"
        if not r.get("case_native_roi_probe"):
            baseline_map[key] = r
    ...
    lines.append("")
    lines.append("| case | native_roi_probe | ok | selected_transport | selected_codec | frames | fps_avg | delta_fps_vs_off | p95(ms) | delta_p95_vs_off(ms) | delta_cpu_mean_vs_off(%) | delta_gpu_mean_vs_off(%) | >100ms | agent_errors | quic_drop |")
    lines.append("|---|---:|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for r in payload.get("cases", []):
        ...
        r["fps_avg"] = fps_avg
        lines.append(
            "| {case} | {native} | {ok} | {transport} | {codec} | {frames} | {fps} | {dfps} | {p95} | {dp95} | {dcpu} | {dgpu} | {g100} | {ae} | {qd} |".format(
