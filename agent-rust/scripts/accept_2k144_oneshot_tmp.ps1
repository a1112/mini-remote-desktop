...
$bak=Join-Path $agentDir ('config.accept_2k144.oneshot.'+(Get-Date -Format 'yyyyMMdd_HHmmss')+'.bak.json')
...
function GA($p){$o=[ordered]@{send=-1.0;unique=-1.0;tier='';reason=''}; foreach($line in Get-Content $p){$t=[regex]::Replace($line,"\x1B\[[0-9;]*m",""); if($t -notmatch '\[RTCP-PANEL\]'){continue}; $m=[regex]::Match($t,'send_fps[^0-9]*([0-9]+(?:\.[0-9]+)?)'); if($m.Success){$v=[double]$m.Groups[$m.Groups.Count-1].Value; if($v -gt $o.send){$o.send=$v}}; $m=[regex]::Match($t,'unique_send_fps[^0-9]*([0-9]+(?:\.[0-9]+)?)'); if($m.Success){$v=[double]$m.Groups[$m.Groups.Count-1].Value; if($v -gt $o.unique){$o.unique=$v}}; $m=[regex]::Match($t,'tier_level=([0-9]+)'); if($m.Success){$o.tier=$m.Groups[1].Value}; $m=[regex]::Match($t,'tier_reason="([^"]+)"'); if($m.Success){$o.reason=$m.Groups[1].Value} }; $o}
try {
$cfg=Get-Content $cfgPath -Raw | ConvertFrom-Json
$cfg.capture.target_width=2560; $cfg.capture.target_height=1440
$cfg.capture.fps=144; $cfg.capture.min_fps=144; $cfg.capture.max_fps=144
$cfg.capture.max_fps_mode=$true; $cfg.capture.idle_repeat_fps=144
$cfg.capture.encoder='auto'
($cfg|ConvertTo-Json -Depth 100)|Set-Content -Path $cfgPath -Encoding Ascii
$tag='accept.2k144.oneshot.'+(Get-Date -Format 'HHmmss')
...
$p=GP $plog; $a=GA $alog
Write-Output ("2k144 probe={0:N2} send={1:N2} unique={2:N2} frames={3} tier={4} reason={5}" -f $p.fps,$a.send,$a.unique,$p.frames,$a.tier,$a.reason)
} finally {
