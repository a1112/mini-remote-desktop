...
$bak=Join-Path $agentDir ('config.wgc.stress.'+(Get-Date -Format 'yyyyMMdd_HHmmss')+'.bak.json')
Copy-Item $cfgPath $bak -Force
$tag='accept.wgc.stress.'+(Get-Date -Format 'HHmmss')
...
try {
  ...
  $np.Refresh()
  if($np.MainWindowHandle -eq 0){ Start-Sleep -Seconds 1; $np.Refresh() }
  if($np.MainWindowHandle -eq 0){ throw 'notepad hwnd unavailable' }
  $hwnd = ('0x{0:X}' -f [int64]$np.MainWindowHandle)
...
"@
    ...
    [WinApi]::ShowWindow($ptr,6) | Out-Null # minimize
    Start-Sleep -Seconds 4
    [WinApi]::ShowWindow($ptr,9) | Out-Null # restore
