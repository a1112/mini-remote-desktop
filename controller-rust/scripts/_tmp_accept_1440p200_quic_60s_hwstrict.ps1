...
try {
  ...
  $cp = Start-Process -FilePath 'cmd.exe' `
    -ArgumentList '/c',("set MRD_TRANSPORT=quic&& set MRD_DECODER=d3d11va&& set MRD_REQUIRE_D3D11VA=1&& set MRD_HARDWARE_FAIL_FAST=1&& set MRD_TRY_MF_FALLBACK=0&& set RUST_LOG=controller_rust=info,tokio=warn,webrtc=warn&& `"$controllerExe`"") `
