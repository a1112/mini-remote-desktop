...
describe("RemoteDisplayWindowPage", () => {
  ...
  it("coalesces high frequency pointer moves to the latest animation frame", async () => {
    ...
    fireEvent.pointerMove(renderArea, { clientX: 320, clientY: 236 });
    fireEvent.pointerMove(renderArea, { clientX: 640, clientY: 416 });
    fireEvent.pointerMove(renderArea, { clientX: 960, clientY: 596 });

    expect(frameCallbacks).toHaveLength(1);
    expect(
      mockInvoke.mock.calls.filter(([command]) => command === "ipc_send_control_input")
    ).toHaveLength(0);

    const flushFrame = frameCallbacks[0];
    if (!flushFrame) throw new Error("expected scheduled animation frame");
    act(() => {
      flushFrame(16);
    });
    ...
    await waitFor(() => {
      ...
      expect(inputCalls.map(([, args]) => args)).toEqual([
        {
          sessionId: "p2p-quic-123",
          event: { kind: "mouse_move", x: 1920, y: 1080 },
        },
      ]);
    });
  });
  ...
  it("releases active remote input when a pointer sequence is cancelled", async () => {
    const mockInvoke = getMockInvoke();
    mockInvoke.mockImplementation((command: string, args?: Record<string, unknown>) => {
      if (command === "test_get_capabilities") {
        return Promise.resolve({
    ...
    fireEvent.pointerDown(renderArea, { button: 0, clientX: 640, clientY: 416 });
    fireEvent.pointerCancel(renderArea);
  ...
  it("suppresses repeated keydown until the key is released", async () => {
    const mockInvoke = getMockInvoke();
    mockInvoke.mockImplementation((command: string, args?: Record<string, unknown>) => {
      if (command === "test_get_capabilities") {
        return Promise.resolve({
    ...
    fireEvent.keyDown(renderArea, { key: "a", code: "KeyA" });
    fireEvent.keyDown(renderArea, { key: "a", code: "KeyA", repeat: true });
    fireEvent.keyDown(renderArea, { key: "a", code: "KeyA", repeat: true });
    fireEvent.keyUp(renderArea, { key: "a", code: "KeyA" });
    fireEvent.keyDown(renderArea, { key: "a", code: "KeyA" });
