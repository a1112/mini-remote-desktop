 ...
 * D3D11-NVENC 动态加载编码器
 *
 * 不依赖 NVENC SDK 头文件，运行时动态加载 NVENC API
 * 使用 D3D11 互操作 (CUDA 11.0+ 支持)
 */
#pragma once
...
struct NVENCEncodeConfig {
    ...
    int rc_mode;  // 0=constqp, 1=vbr, 2=cbr, 4=vbrminqp
};
