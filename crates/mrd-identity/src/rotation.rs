...
impl DeviceIdentity {
    pub fn sign_rotation(
        ...
        let payload = RotationPayload {
            ...
            epoch,
            old_public_key: self.public_key().to_vec(),
        };
        let bytes = canonical_bytes("MRD_KEY_ROTATION_V1", &payload)
...
impl RotationProof {
    pub fn verify(
        ...
        if revoked {
            return Err(RotationError::OldKeyRevoked);
        }
        if self.epoch <= current_epoch {
            return Err(RotationError::EpochNotIncreasing);
        }
        if self.old_public_key != old_public_key {
            return Err(RotationError::InvalidSignature);
        }
        let payload = RotationPayload {
            new_public_key: self.new_public_key.clone(),
            epoch: self.epoch,
            old_public_key: self.old_public_key.clone(),
        };
        let bytes = canonical_bytes("MRD_KEY_ROTATION_V1", &payload)
            .map_err(|_| RotationError::InvalidSignature)?;
        signature::UnparsedPublicKey::new(&signature::ED25519, old_public_key)
