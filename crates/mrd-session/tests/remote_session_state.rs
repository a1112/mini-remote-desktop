...
use mrd_session::{
    AuthorizationState, CapabilitySet, MediaState, RemoteSessionAggregate, RouteKind, RouteState,
    SessionPlan, SessionTransitionError,
};
...
fn route_migration_preserves_granted_scopes() {
    let mut session = RemoteSessionAggregate::new(fixture_plan());
    session
        .authorize(vec!["screen_view".into(), "input_pointer".into()], 3)
        .unwrap();
    session.begin_route_migration(RouteKind::LanQuic).unwrap();
    session
        .complete_route_migration(RouteKind::LanQuic)
        .unwrap();
    let scopes = session.granted_scopes().to_vec();
    session
        .begin_route_migration(RouteKind::WebRtcRelay)
        .unwrap();
    session
        .complete_route_migration(RouteKind::WebRtcRelay)
        .unwrap();
    assert_eq!(session.granted_scopes(), scopes.as_slice());
    assert_eq!(
        session.route_state(),
        &RouteState::Active(RouteKind::WebRtcRelay)
    );
}
