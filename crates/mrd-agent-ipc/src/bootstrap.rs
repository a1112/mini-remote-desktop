...
use crate::{RegistrationProofVerifier, AGENT_IPC_MAX_IDENTIFIER_BYTES};
...
const BOOTSTRAP_MAGIC: &[u8; 8] = b"MRDABT1\0";
const BOOTSTRAP_VERSION: u16 = 1;
const BOOTSTRAP_HEADER_BYTES: usize = 104;
...
impl ReceivedAgentBootstrap {
    ...
    pub fn into_parts(self) -> (String, u32, u64, u32, u32, [u8; 32], Zeroizing<[u8; 32]>) {
        (
            self.control_endpoint,
            self.service_process_id,
            self.service_process_creation_time,
...
where
    W: AsyncWrite + Unpin,
{
    validate_bootstrap_fields(
        bootstrap.control_endpoint,
        bootstrap.service_process_id,
        bootstrap.service_process_creation_time,
        bootstrap.heartbeat_interval_ms,
        bootstrap.handshake_timeout_ms,
        &bootstrap.registration_seed,
        &bootstrap.expected_agent_key_id,
    )?;
...
pub fn windows_agent_bootstrap_pipe_name(
    ...
    format!(
        r"\\.\pipe\mrd-agent-bootstrap-v1-s{windows_session_id}-p{process_id}-c{process_creation_time:016x}"
    )
}

fn validate_bootstrap_fields(
    control_endpoint: &str,
    service_process_id: u32,
    service_process_creation_time: u64,
    heartbeat_interval_ms: u32,
    handshake_timeout_ms: u32,
    registration_seed: &[u8; 32],
    expected_agent_key_id: &[u8; 32],
) -> Result<(), AgentBootstrapError> {
    if control_endpoint.is_empty()
        || control_endpoint.contains('\0')
        || control_endpoint.len() > AGENT_IPC_MAX_IDENTIFIER_BYTES * 2
        || service_process_id == 0
        || service_process_creation_time == 0
        || heartbeat_interval_ms == 0
        || handshake_timeout_ms == 0
        || expected_agent_key_id.iter().all(|byte| *byte == 0)
    {
        return Err(AgentBootstrapError::InvalidRecord);
    }
    let derived = derive_registration_public_key(registration_seed)?;
    if &derived.key_id != expected_agent_key_id {
        return Err(AgentBootstrapError::InvalidRegistrationKey);
    }
    Ok(())
}
