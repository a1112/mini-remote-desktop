...
fn transport_receiver_reassembles_fua_into_h264_access_unit() {
    ...
    assert_eq!(access_unit.bytes, vec![0, 0, 0, 1, 0x65, 0xaa, 0xbb, 0xcc, 0xdd]);
}
...
fn transport_receiver_accumulates_stap_a_and_fua_until_marker() {
    ...
    assert!(ingress
        .push_payload(
            &[24, 0, 2, 0x67, 0x42, 0, 2, 0x68, 0xce],
            false,
            66_000,
        )
        .is_none());
    assert!(ingress.push_payload(&[0x7c, 0x85, 0xaa, 0xbb], false, 66_000).is_none());
