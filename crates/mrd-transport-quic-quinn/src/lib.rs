...
/// Ephemeral QUIC server cryptographic material that has not opened a listener yet.
pub struct QuinnPreparedServer {
    server_config: ServerConfig,
    server_name: String,
    cert_der: Vec<u8>,
}

impl QuinnPreparedServer {
    /// Generates the certificate and server configuration without binding a socket.
    pub fn generate() -> Result<Self, QuinnTransportError> {
        let server_crypto =
    ...
    /// Opens a listener using the exact certificate and configuration generated by `generate`.
    pub async fn bind(
        self,
        bind_addr: &str,
    ) -> Result<(QuinnServerListener, QuinnServerBootstrap), QuinnTransportError> {
        let Self {
            server_config,
            server_name,
            cert_der,
...
impl QuinnServerListener {
    ...
    pub async fn bind(
        ...
        QuinnPreparedServer::generate()?.bind(bind_addr).await
    }
...
impl QuinnDatagramEndpoint {
    ...
    pub fn close_immediately(&self, reason: &[u8]) {
        self.inner.connection.close(0_u32.into(), reason);
        self.inner.endpoint.close(0_u32.into(), reason);
    }
...
mod tests {
    ...
    async fn assert_close_rejects_partial_reliable_message(persistent: bool) {
        let pair = QuinnDatagramPair::loopback()
            .await
            .expect("partial reliable message endpoint pair");
        let sending_endpoint = pair.client.clone();
        let payload = Bytes::from(vec![0xD3; 16 * 1024 * 1024]);
        let mut send = tokio::spawn(async move {
            if persistent {
                sending_endpoint
                    .send_reliable_message_persistent(payload)
                    .await
            } else {
                sending_endpoint.send_reliable_message(payload).await
            }
        });

        let mut stream = timeout(
            Duration::from_secs(1),
            pair.server.inner.connection.accept_uni(),
        )
        .await
        .expect("peer accepts reliable stream before cancellation")
        .expect("accepted reliable stream");
        let mut prefix = [0_u8; 1_024];
        timeout(Duration::from_secs(1), stream.read_exact(&mut prefix))
            .await
            .expect("peer reads a reliable prefix before cancellation")
            .expect("reliable prefix");
        assert!(
            timeout(Duration::from_millis(50), &mut send).await.is_err(),
            "the remaining reliable payload must still be backpressured"
        );

        pair.client
            .close_immediately(b"authorization no longer permits media");
        let peer_result = timeout(Duration::from_secs(1), stream.read_to_end(32 * 1024 * 1024))
            .await
            .expect("peer read resolves after connection close");
        assert!(
            peer_result.is_err(),
            "connection close must reset a partial reliable message instead of returning Ok(partial)"
        );
        let send_result = timeout(Duration::from_secs(1), send)
            .await
            .expect("reliable sender resolves after connection close")
            .expect("reliable sender task");
        assert!(send_result.is_err());
    }

    #[tokio::test]
    async fn close_before_cancelling_per_message_send_resets_the_partial_stream() {
        assert_close_rejects_partial_reliable_message(false).await;
    }
