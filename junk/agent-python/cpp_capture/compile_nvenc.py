...
# 源文件
SRC_FILE = "nvenc_d3d12_encoder.cpp"
DLL_NAME = "nvenc_d3d12_encoder.dll"
...
include_dirs = [
    f"/I\"{CUDA_INCLUDE}\"",
    f"/I\"{CUDA_INCLUDE}\\..\"",  # 有时候 include 在父目录
]
...
if result.stderr:
    for line in result.stderr.split('\n')[-30:]:
        if line.strip() and 'Initializing' not in line and 'environment' not in line.lower():
            print(f"  {line}")
...
print(f"\n使用方法:")
print(f"  Python: from encoder.nvenc_d312_encoder import NVENC_Encoder")
print(f"  C++:   #include \"nvenc_d3d12_encoder.h\"")
...
dll.is_cuda_d3d12_interop_supported.argtypes = []
dll.is_cuda_d3d12_interop_supported.restype = ctypes.c_int

nvenc_sup = dll.is_nvenc_supported()
cuda_sup = dll.is_cuda_d3d12_interop_supported()

print(f"  NVENC 支持: {'✅' if nvenc_sup else '❌'}")
print(f"  CUDA-D3D12 互操作: {'✅' if cuda_sup else '❌'}")
print(f"  编码器状态: {'✅ 就绪' if nvenc_sup else '❌ 需要检查驱动'}")
