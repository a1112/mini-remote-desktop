...
$agentBak=Join-Path $agentDir ('config.webrtc_swenc.' + (Get-Date -Format 'yyyyMMdd_HHmmss') + '.bak.json')
...
try {
  foreach($cap in $captureBackends){
    ...
    $tag="accept.webrtc.swenc_swdec.$cap." + (Get-Date -Format 'HHmmss')
    ...
    Write-Output ("webrtc+swenc+swdec cap={0} no_output={1} recover={2} pli={3} total={4} decoded={5} ctl_fps={6} encode_fps={7} send_fps={8}" -f $cap,$noOut,$recover,$pli,$totalFrames,$decodedFrames,[math]::Round($ctrlFps,2),[math]::Round($encodeFps,2),[math]::Round($sendFps,2))
