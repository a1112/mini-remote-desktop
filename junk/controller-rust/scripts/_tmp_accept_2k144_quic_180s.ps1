...
$agentBak = Join-Path $agentDir ('config.accept.quic2k144.' + (Get-Date -Format 'yyyyMMdd_HHmmss') + '.bak.json')
...
try {
  ...
  $cfg.capture.fps = 144
  $cfg.capture.min_fps = 144
  $cfg.capture.max_fps = 144
  $cfg.capture.max_fps_mode = $true
  $cfg.capture.idle_repeat_fps = 144
  ...
  $tag = "accept.quic.2k144." + (Get-Date -Format 'HHmmss')
  ...
  $ap = Start-Process -FilePath 'cmd.exe' `
    -ArgumentList '/c',("set AGENT_FFMPEG_PATH=$ffmpegExe&& set AGENT_QUIC_DEBUG=1&& set AGENT_QUIC_QUEUE=128&& set AGENT_QUIC_MAX_AU_BYTES=2097152&& `"$agentExe`"") `
  ...
  $cp = Start-Process -FilePath 'cmd.exe' `
    -ArgumentList '/c',("set MRD_TRANSPORT=quic&& set MRD_DECODER=mf_d3d11&& set RUST_LOG=controller_rust=info,tokio=warn,webrtc=warn&& `"$controllerExe`"") `
