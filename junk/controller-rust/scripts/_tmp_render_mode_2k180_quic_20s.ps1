...
try {
  ...
  foreach($mode in @('low_latency','smooth')){
    ...
    $ccmd='/c set MRD_TRANSPORT=quic&& set MRD_DECODER=d3d11va&& set MRD_RENDER_MODE=' + $mode + '&& set MRD_SMOOTH_TARGET_FPS=120&& set RUST_LOG=controller_rust=info,tokio=warn,webrtc=warn&& "' + $controllerExe + '"'
