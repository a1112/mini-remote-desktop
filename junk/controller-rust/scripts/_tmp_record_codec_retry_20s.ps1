...
$controllerBak=Join-Path $controllerDir ('config.record.codec.retry.' + (Get-Date -Format 'yyyyMMdd_HHmmss') + '.bak.json')
$agentCfgPath=Join-Path $agentDir 'config.json'
$agentBak=Join-Path $agentDir ('config.record.codec.retry.' + (Get-Date -Format 'yyyyMMdd_HHmmss') + '.bak.json')
...
try {
  ...
  Set-JsonField $acfg.capture 'backend' 'dxgi'; Set-JsonField $acfg.capture 'encoder' 'nvenc'
  Set-JsonField $acfg.capture 'strict_gpu_direct' $true; Set-JsonField $acfg.capture 'allow_fallback' $false; Set-JsonField $acfg.capture 'allow_encoder_fallback' $false
  ($acfg | ConvertTo-Json -Depth 100) | Set-Content -Path $agentCfgPath -Encoding Ascii

  foreach($codec in $codecs){
    $dir=Join-Path $base ('recordings.codec.retry.' + $codec + '.' + (Get-Date -Format 'HHmmss'))
    ...
    Set-JsonField $ccfg.record 'input_fps' 120
    Set-JsonField $ccfg.record 'container' 'mp4'
    Set-JsonField $ccfg.record 'video_codec' $codec
    Set-JsonField $ccfg.record 'video_preset' 'auto'
    Set-JsonField $ccfg.record 'video_crf' 24
    Set-JsonField $ccfg.record 'video_bitrate_kbps' 8000
    Set-JsonField $ccfg.record 'queue_depth' 2048
    ($ccfg | ConvertTo-Json -Depth 100) | Set-Content -Path $controllerCfgPath -Encoding Ascii

    $tag='accept.record.codec.retry.' + $codec + '.' + (Get-Date -Format 'HHmmss')
    ...
    $written=Extract $r 'written_frames=([0-9]+)'; $wf=Extract $r 'write_failures=([0-9]+)'
    $files=@(); if(Test-Path $dir){ $files=Get-ChildItem -Path $dir -File -Filter '*.mp4' }
    Write-Output ("retry codec={0} written_frames={1} write_failures={2} mp4_count={3} dir={4} clog={5}" -f $codec,$written,$wf,$files.Count,$dir,$clog)
  }
}
finally {
