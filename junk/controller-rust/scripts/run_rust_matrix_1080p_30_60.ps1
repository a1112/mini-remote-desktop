...
$bak = Join-Path $Base ('agent-rust/config.lowfps.' + (Get-Date -Format 'yyyyMMdd_HHmmss') + '.bak.json')
...
function Run-Case([int]$fps, [int]$queue, [int]$renderMaxAgeMs) {
  Set-Capture -fps $fps
  Stop-Mrd
  $tag = ('rust.lowfps1080p.f' + $fps + '.q' + $queue + '.r' + $renderMaxAgeMs + '.' + (Get-Date -Format 'HHmmss'))
