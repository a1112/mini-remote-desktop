...
$agentBak = Join-Path $agentDir ('config.accept.quic240.' + (Get-Date -Format 'yyyyMMdd_HHmmss') + '.bak.json')
...
try {
  ...
  $cfg.capture.target_width = 0
  $cfg.capture.target_height = 0
  $cfg.capture.fps = 240
  ...
  $tag = "accept.quic.1080p240." + (Get-Date -Format 'HHmmss')
