...
$agentBak = Join-Path $agentDir ('config.accept.quic240.gpudirect.extreme.' + (Get-Date -Format 'yyyyMMdd_HHmmss') + '.bak.json')
...
function Apply-ExtremeProfile($cfg, [int]$targetW, [int]$targetH) {
  $cfg.capture.fps = 240
  $cfg.capture.min_fps = 240
  $cfg.capture.max_fps = 240
  $cfg.capture.max_fps_mode = $true
  $cfg.capture.idle_repeat_fps = 240
  $cfg.capture.target_width = $targetW
  $cfg.capture.target_height = $targetH
...
function Run-Case([string]$name, [int]$targetW, [int]$targetH, [int]$seconds = 20) {
  $cfg = Get-Content $agentCfgPath -Raw | ConvertFrom-Json
  Apply-ExtremeProfile -cfg $cfg -targetW $targetW -targetH $targetH
  ($cfg | ConvertTo-Json -Depth 100) | Set-Content -Path $agentCfgPath -Encoding Ascii

  $tag = "accept.quic.1080p240.gpudirect.extreme.$name." + (Get-Date -Format 'HHmmss')
  ...
  Write-Output ("case={0} target={1}x{2} quic_connected={3} controller_fps={4} avg_decode_ms={5} p95_decode_ms={6} jitter_ms={7}" -f `
    $name,$targetW,$targetH,$cs.connected_quic,([math]::Round($cs.fps,2)),([math]::Round($cs.avg_decode_ms,3)),([math]::Round($cs.p95_decode_ms,3)),([math]::Round($cs.jitter_ms,3)))
  Write-Output ("case={0} send_fps={1} unique_fps={2} quic_au_sent={3} quic_au_dropped={4} quic_bytes_sent={5}" -f `
    $name,([math]::Round($as.send_fps,2)),([math]::Round($as.unique_fps,2)),$as.quic_au_sent,$as.quic_au_dropped,$as.quic_bytes_sent)
  Write-Output ("case={0} native_direct={1} native_copy={2} native_scale={3} native_direct_reg_fail={4}" -f `
    $name,$as.native_direct_frames,$as.native_copy_frames,$as.native_scale_frames,$as.native_direct_register_failures)
  Write-Output ("case={0} logs: agent={1} controller={2} signaling={3}" -f $name,$alog,$clog,$slog)
}

try {
  Run-Case -name 'extreme_noscale' -targetW 0 -targetH 0
  Run-Case -name 'extreme_1080' -targetW 1920 -targetH 1080
}
finally {
