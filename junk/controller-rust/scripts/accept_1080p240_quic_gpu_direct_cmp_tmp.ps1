...
$agentBak = Join-Path $agentDir ('config.accept.quic240.gpudirectcmp.' + (Get-Date -Format 'yyyyMMdd_HHmmss') + '.bak.json')
...
function Run-Case([string]$name, [int]$targetW, [int]$targetH) {
  ...
  $tag = "accept.quic.1080p240.gpudirect.$name." + (Get-Date -Format 'HHmmss')
...
try {
  Run-Case -name 'direct_noscale' -targetW 0 -targetH 0
  Run-Case -name 'direct_1080' -targetW 1920 -targetH 1080
}
finally {
